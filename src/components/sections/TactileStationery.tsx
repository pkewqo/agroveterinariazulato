import React from 'react';
import { useBrand } from '../../context/BrandContext';
import { BrandLogo } from '../BrandLogo';

export const TactileStationery: React.FC = () => {
  const { brand } = useBrand();

  return (
    <section id="stationery" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
        
        {/* Section Title */}
        <div className="flex flex-wrap items-baseline justify-between gap-4 border-b border-stone-800 pb-6">
          <div className="space-y-1">
            <span className="text-xs font-mono font-bold text-emerald-400 uppercase tracking-widest block">
              Seção 05 • Papelaria & Aplicações
            </span>
            <h2 
              className="text-3xl sm:text-4xl font-normal text-stone-100 tracking-tight"
              style={{ fontFamily: '"Playfair Display", Georgia, serif' }}
            >
              Papelaria Corporativa & Mostruário Físico
            </h2>
          </div>
          <p className="text-xs font-mono text-stone-400">
            Cartão de Visita 90×50mm • Papel Timbrado A4
          </p>
        </div>

        {/* 1. Business Card */}
        <div className="space-y-8">
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-stone-200 flex items-center gap-2">
              <span className="font-mono text-emerald-400">5.1</span>
              <span>Cartão de Visita Corporativo (90 × 50 mm)</span>
            </h3>
            <p className="text-xs text-stone-400">
              Especificação física: Papel Couché Fosco 350g/m², laminação Soft Touch bilateral e verniz reserva UV localizado.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            
            {/* Front */}
            <div className="space-y-2">
              <span className="text-[11px] font-mono text-stone-400 uppercase font-semibold block">
                Frente (Contato & Responsabilidade Técnica)
              </span>
              <div 
                className="p-8 flex flex-col justify-between aspect-[9/5] min-h-[220px] bg-stone-100 text-stone-900 paint-chip-shadow"
              >
                <div className="flex justify-between items-start">
                  <BrandLogo variant="horizontal" colorMode="color-light" size="sm" />
                  <span className="text-[9px] font-mono font-bold text-emerald-900 uppercase tracking-wider bg-emerald-200/80 px-2 py-0.5">
                    30 anos de história!
                  </span>
                </div>

                <div className="space-y-1">
                  <p className="text-base font-bold text-stone-900 leading-tight">
                    Dr. Marcos Zulato
                  </p>
                  <p className="text-xs text-stone-700 font-medium">
                    Médico Veterinário & Responsável Técnico
                  </p>
                  <p className="text-[11px] font-mono text-emerald-800 font-semibold">
                    CRMV-SP 18.420 • Nutrição & Sanidade Animal
                  </p>
                </div>

                <div className="pt-2 flex justify-between items-center text-[10px] font-mono text-stone-600 border-t border-stone-300">
                  <span>{brand.socialHandles.location}</span>
                  <span className="font-bold text-emerald-900">{brand.socialHandles.phone}</span>
                </div>
              </div>
            </div>

            {/* Back */}
            <div className="space-y-2">
              <span className="text-[11px] font-mono text-stone-400 uppercase font-semibold block">
                Verso (Assinatura Institucional Gravada)
              </span>
              <div 
                className="p-8 flex flex-col justify-between aspect-[9/5] min-h-[220px] paint-chip-shadow text-white"
                style={{ backgroundColor: brand.colors.neutralDark.hex }}
              >
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-mono tracking-widest text-emerald-400 font-bold uppercase">
                    ★ 30 anos de história!
                  </span>
                  <span className="text-[9px] font-mono tracking-widest text-stone-400 uppercase">
                    Dracena - SP
                  </span>
                </div>

                <div className="flex flex-col items-center justify-center">
                  <BrandLogo variant="vertical" colorMode="color-dark" />
                </div>

                <div className="text-center text-[9px] font-mono text-stone-400">
                  {brand.socialHandles.location} • Tel: {brand.socialHandles.phone}
                </div>
              </div>
            </div>

          </div>
        </div>

        {/* 2. Letterhead */}
        <div className="space-y-8">
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-stone-200 flex items-center gap-2">
              <span className="font-mono text-emerald-400">5.2</span>
              <span>Papel Timbrado Institucional A4 (210 × 297 mm)</span>
            </h3>
            <p className="text-xs text-stone-400">
              Especificação física: Papel Vergê 120g/m² com marca d'água táctil. Impressão 4x0 cores.
            </p>
          </div>

          <div className="max-w-3xl mx-auto bg-stone-100 text-stone-900 p-10 sm:p-14 space-y-12 paint-chip-shadow">
            
            {/* Header */}
            <div className="flex justify-between items-start pb-6 border-b-2" style={{ borderColor: brand.colors.primary.hex }}>
              <div>
                <BrandLogo variant="horizontal" colorMode="color-light" size="sm" />
                <p className="text-[10px] font-mono font-bold text-emerald-900 mt-1 uppercase">
                  30 anos de história! • {brand.category}
                </p>
              </div>
              <div className="text-right text-[10px] font-mono text-stone-600 space-y-0.5">
                <p className="font-bold text-stone-900 text-xs">{brand.brandName}</p>
                <p>{brand.socialHandles.location}</p>
                <p className="font-bold text-emerald-900">WhatsApp / Tel: {brand.socialHandles.phone}</p>
              </div>
            </div>

            {/* Body */}
            <div className="space-y-4 text-xs text-stone-700 leading-relaxed min-h-[140px]">
              <div className="flex justify-between text-[11px] font-semibold text-stone-900 pb-2">
                <span>DESTINATÁRIO: Agropecuária & Produtores Rurais Associados</span>
                <span>DATA: 17 de Agosto de 2026</span>
              </div>
              
              <h5 className="text-base font-bold text-stone-900 pt-2">
                Parecer Técnico & Diretrizes Sanitárias de Manejo
              </h5>

              <p>
                Atestamos para os devidos fins que os procedimentos de consultoria zootécnica e os insumos fornecidos pela <strong>{brand.brandName}</strong> atendem rigorosamente aos padrões de qualidade farmacêutica, pureza mineral e conformidade veterinária, respaldados por <strong>30 anos de história e tradição em Dracena e região</strong>.
              </p>

              <div className="p-4 bg-white border border-stone-300 space-y-1 font-mono text-[11px]">
                <p className="font-bold text-stone-900">RECOMENDAÇÃO TÉCNICA OFICIAL:</p>
                <p>1. Suplementação balanceada com controle de ganho de peso diário (GMD).</p>
                <p>2. Manejo preventivo e sanitário certificado.</p>
              </div>
            </div>

            {/* Footer */}
            <div className="pt-6 flex justify-between items-end text-[10px] text-stone-500 font-mono border-t border-stone-300">
              <div>
                <p className="font-bold text-stone-900">Dr. Marcos Zulato</p>
                <p>CRMV-SP 18.420 • Responsável Técnico</p>
              </div>
              <div className="text-right">
                <p>{brand.socialHandles.location} • {brand.socialHandles.phone}</p>
              </div>
            </div>

          </div>
        </div>

      </div>
    </section>
  );
};
